...
def compute_rerank_provenance(reranker, query, documents, answer):
    ...
    # Log original docs passed to reranker
    for i, doc in enumerate(documents):
        logger.info("[RERANK INPUT] Doc #%d content preview: %s", i, doc.page_content[:200])
        logger.info("[RERANK INPUT] Doc #%d metadata: %s", i, doc.metadata)
    ...
    for i, doc in enumerate(scored_documents):
        ...
        logger.info("[RERANK OUTPUT] Doc #%d score: %.4f | metadata: %s", i, score, doc.metadata)

    return scored_documents


def compute_llm_provenance(tokenizer, model, query, context, answer):
    logger.info("PROVENANCE FUNCTION ENTERED")
    ...
    for doc in context:
        ...
        rendered_prompt = prompt.format_map({"query": query, "context": new_doc, "answer": answer})
        logger.debug("FULL PROMPT to LLM:\n%s", rendered_prompt)

        context_thread = [{'role': 'user', 'content': rendered_prompt}]
        input_chat = tokenizer.apply_chat_template(context_thread, tokenize=False)
        logger.debug("Full chat-formatted input: %s", input_chat)
        ...
        try:
            index = generated_text.rindex(end_string)
            answer = generated_text[index + len(end_string):].strip()
            if not answer:
                raise ValueError("Empty answer after marker.")
        except ValueError:
            logger.warning("Assistant token not found or empty answer — using full output as fallback.")
            answer = generated_text.strip()

        matches = re.findall(r"\d+\.?\d*", answer.replace(",", "."))
        if not matches:
            logger.warning("[NO MATCHES] answer = %r", answer)

        score = float(matches[-1]) if matches else float("nan")
        logger.debug("LLM Response (raw): %s", generated_text)
        logger.debug("Parsed answer part: %s", answer)
        logger.debug("Regex matches found: %s", matches)
        logger.info("Final provenance score for doc: %.3f", score)
...
def compute_llm_provenance_cloud(llm, query, context, answer):
    ...
    for doc in context:
        ...
        print("LLM Response (raw):", generated_text)
        if hasattr(generated_text, 'content'):
            ...
            generated_text = generated_text["answer"]
        print("Parsed answer part:", generated_text)
        matches = re.findall(r"\d+\.?\d*", answer.replace(",", "."))
